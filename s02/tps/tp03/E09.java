...
class E09 {
    public static void main(String[] args) throws IOException {
        ...
        try (Scanner s = new Scanner(System.in)) {
            ...
            mergeSort(selected, (a, b) -> {
                int res = Float.compare(a.getUpvotes(), b.getUpvotes());
                if (res != 0) {
                    return res;
                }
    ...
    private static <T> void mergeSort(List<T> list, Comparator<? super T> c) {
        mergeSort(list, c, 0, list.size() - 1);
    }

    private static <T> void mergeSort(List<T> list, Comparator<? super T> c, int l, int r) {
        if (l < r) {
            int m = l + (r - l) / 2;
            mergeSort(list, c, l, m);
            mergeSort(list, c, m + 1, r);
            merge(list, c, l, m, r);
        }
    }

    private static <T> void merge(List<T> list, Comparator<? super T> c, int l, int m, int r) {
        // Sub-lists initialization
        int n1 = m - l + 1;
        int n2 = r - m;
...
class Game {
    ...
    public float getUpvotes() {
        float first = getUpvotes1();
        float second = getUpvotes2();
