...
class E05 {
    public static void main(String[] args) throws IOException {
        ...
        try (Scanner s = new Scanner(System.in)) {
            ...
            h.extend(selected); // Fill the heap.

            List<Game> sorted = h.heapSort();
...
class Heap<T> {
    ...
    public void extend(List<T> initial) {
        for (T el : initial) {
            insert(el);
        }
    }
    ...
    private void insert(T value) {
        // This may lead to UB if called after an unaccounted `logicalRemove`
        // since `buf.add` would insert the new element AFTER the
        // logically-removed one!
        buf.add(value);
        int curr = len++;

        // While the current element is not the root (i.e. > 0) and is bigger
        // than its parent, one swaps.
        int parent;
        while (curr > 0 && isHigher(curr, parent = parent(curr))) {
            swap(curr, parent);
            curr = parent;
        }
    }
    ...
    public Optional<T> remove() {
        ...
        logicalRemove();
        // `len` is decremented by `logicalRemove`. The following is non-null.
        return Optional.of(buf.remove(len));
    }

    /**
     * Performs a heap sort over this heap's internal buffer.
     *
     * Notice that, after this method is being called, the heap is logically
     * destroyed. Hence, it is UNDEFINED BEHAVIOR to use any other heap methods
     * in the same instance afterwards.
     *
     * P.S. How I wish Java had affine types. XD
     */
    public List<T> heapSort() {
        // One iterates while there is more than one element in the heap.
        //
        // The last remaining element will be the smallest, so it would already
        // be in the right position.
        while (len > 1) {
            // See the `logicalRemove` implementation to understand.
            //
            // In essence, it removes the heap's root, putting it at the
            // buffer's end. It logically decrements the heap size, but the
            // element remains physically there.
            //
            // This sorts the buffer since the greatest elements are being put
            // at the end.
            logicalRemove();
        }
        return buf;
    ...
    /**
     * Puts the root at the last `buf` element and decrements the heap size.
     * This method may be useful for the `Heap::{remove, sort}` methods.
     */
    private void logicalRemove() {
        if (len == 0) {
            return;
        }
        // First, one swaps the first element with the last, so that it may be
        // more easily removed (i.e. without any shifts). This statement also
        // decrements the heap size.
        swap(0, --len);

        // Readjust the remaining heap.
        int parent = 0;
        OptionalInt opt;
        while ((opt = biggestChild(parent)).isPresent()) {
            int child = opt.getAsInt();
            if (isHigher(child, parent)) {
                // One swaps the child with the parent if the former is bigger.
                swap(child, parent);
                parent = child;
            } else {
                // Otherwise, the parent is already bigger than both of its
                // children, so the loop is terminated.
                break;
            }
        }
    }
    ...
    private int parent(int index) {
        return (index - 1) / 2; // c.f. in paper.
    }

    private boolean isLeaf(int index) {
        return index >= len / 2; // c.f. in paper.
    }
    ...
    private OptionalInt biggestChild(int parent) {
        if (!hasChildren(parent)) {
            return OptionalInt.empty();
        }
        int l = 2 * parent + 1;
        int r = l + 1;
        boolean hasRight = r < len;
        if (!hasRight || isHigher(l, r)) {
            return OptionalInt.of(l);
        } else {
            // Small semantic note: if both children are equal, the `r` index
            // is hereby being returned.
    ...
    /**
     * Checks if the first provided element should be placed "higher" than the
     * second.
     */
    private boolean isHigher(int a, int b) {
        int res = comparator.compare(at(a), at(b));
        return res >= 1;
    }
}
