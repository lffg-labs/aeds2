...
class E08 {
    public static void main(String[] args) throws IOException {
        ...
        try (Scanner s = new Scanner(System.in)) {
            ...
            bubbleSort(selected, (a, b) -> {
                int res = a.getDevelopers().compareTo(b.getDevelopers());
                if (res != 0) {
                    return res;
                }
    ...
    private static <T> void bubbleSort(List<T> list, Comparator<? super T> c) {
        int len = list.size();
        for (int i = (len - 1); i > 0; i--) {
            for (int j = 0; j < i; j++) {
                if (c.compare(list.get(j), list.get(j + 1)) >= 1) {
                    swap(list, j, j + 1);
                }
            }
        }
