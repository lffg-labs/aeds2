...
class E03 {
    public static void main(String[] args) throws IOException {
        ...
        try (Scanner s = new Scanner(System.in)) {
            ...
            insertionSort(selected, (a, b) -> {
                return a.getAppId() - b.getAppId();
            });
    ...
    private static <T> void insertionSort(List<T> list, Comparator<? super T> c) {
        for (int i = 1; i < list.size(); i++) {
            int j = i;
            T curr = list.get(i);
            // Move `curr` DOWN while it is SMALLER than its predecessor.
            while (j - 1 >= 0 && c.compare(curr, list.get(j - 1)) < 0) {
