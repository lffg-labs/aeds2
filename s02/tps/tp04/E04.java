...
class E04 {
    public static void main(String[] args) throws IOException {
        ...
        try (Scanner s = new Scanner(System.in)) {
            LinkedList<Game> selected = new LinkedList<Game>();
            // Fill the array.
            consumeGroup(s, (rawId) -> {
                selected.push(gamesDataset.get(Integer.parseInt(rawId)));
            });
    ...
    private static <T> void quickSort(LinkedList<T> list, Comparator<? super T> c) {
        quickSort(list, c, 0, list.length() - 1);
    }

    private static <T> void quickSort(LinkedList<T> list, Comparator<? super T> c, int left, int right) {
        int l = left;
        int r = right;
