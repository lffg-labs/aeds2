...
class E03 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        LinkedQueue<Game> selectedGames = LinkedQueue.withCapacity(GAMES_QUEUE_SIZE);
        ...
        int i = 0;
        for (Game curr : selectedGames.frontToBackIterator()) {
            System.out.println("[" + (i++) + "] " + curr.toString());
        }
    }

    private static void insertAndPrintAverage(Game g, LinkedQueue<Game> queue) {
        if (queue.isFull()) {
            queue.popBack();
        }
        queue.pushFront(g);
        double total = 0;
        for (Game curr : queue.frontToBackIterator()) {
